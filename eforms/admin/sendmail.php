...
const URL = "http://ithea.test/eforms/token.php?id=";
...
if (empty($_POST['email']) || trim($_POST['email']) == '') {
    ...
    echo json_encode(array('status' => false, 'msg' => "Please Enter Email..!"));
...
if (empty($_POST['link1']) && empty($_POST['link2']) && empty($_POST['link3'])) {
    ...
    echo json_encode(array('status' => false, 'msg' => "Please select a form to send..!"));
...
if (empty($_POST['phone']) || trim($_POST['phone']) == '') {
    ...
    echo json_encode(array('status' => false, 'msg' => "Please Enter Phone Number..!"));
...
if (isset($_POST)) {
    ...
    $token1 = isset($_POST['link1']) && $_POST['link1'] == 1 ? md5(uniqid(rand(), true)) : null;

    $token2 = isset($_POST['link2']) && $_POST['link2'] == 1 ? md5(uniqid(rand(), true)) : null;

    $token3 = isset($_POST['link3']) && $_POST['link3'] == 1 ? md5(uniqid(rand(), true)) : null;

    $phone_otp = rand(10001, 99999);

    $phone_otp2 = rand(10001, 99999);

    $phone_otp3 = rand(10001, 99999);
    ...
    $params = [
        ...
        'link1' => isset($_POST['link1']) && $_POST['link1'] == 1 ? 'Yes' : 'No',

        'token1' => isset($_POST['link1']) && $_POST['link1'] == 1 ? $token1 : null,

        'link2' => isset($_POST['link2']) && $_POST['link2'] == 1 ? 'Yes' : 'No',

        'token2' => isset($_POST['link2']) && $_POST['link2'] == 1 ? $token2 : null,

        'link3' => isset($_POST['link3']) && $_POST['link3'] == 1 ? 'Yes' : 'No',

        'token3' => isset($_POST['link3']) && $_POST['link3'] == 1 ? $token3 : null,

        'link_expiry' => date('Y-m-d H:i:s', strtotime('+7 days')),

        'phone' => $phone,

        'phone_otp' => $phone_otp,

        'phone_otp2' => $phone_otp2,

        'phone_otp3' => $phone_otp3,

        'is_phone_otp_sent' => 0,

        'is_phone_otp2_sent' => 0,

        'is_phone_otp3_sent' => 0
    ...
    $_sql = "INSERT INTO mail_data(`email`,`name`,`link1`,`token1`,`link2`,`token2`,`link3`,`token3`,`link_expiry`,`phone`,`phone_otp`,`phone_otp2`,`phone_otp3`,`is_phone_otp_sent`,`is_phone_otp2_sent`,`is_phone_otp3_sent`) VALUES(:email,:name, :link1, :token1, :link2, :token2, :link3, :token3, :link_expiry, :phone, :phone_otp, :phone_otp2, :phone_otp3, :is_phone_otp_sent, :is_phone_otp2_sent, :is_phone_otp3_sent)";
...
if (isset($_POST['link2']) && $_POST['link2']) {
...
$html .= '<div style="font-size:1px;line-height:22px"> </div><img align="center" alt="I m an image" border="0" class="center fixedwidth" src="http://ithea.test/eforms/images/logo.jpg" style="text-decoration: none; -ms-interpolation-mode: bicubic; height: auto; border: 0; width: 100%; max-width: 160px; display: block;" title="I m an image" width="160"/>';
...
if (isset($_POST['link3']) && $_POST['link3']) {
    ...
    $html .= '<!--[if mso]><table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-spacing: 0; border-collapse: collapse; mso-table-lspace:0pt; mso-table-rspace:0pt;"><tr><td style="padding-top: 10px; padding-right: 10px; padding-bottom: 0px; padding-left: 10px" align="center"><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="' . $url3 . '" style="height:36.75pt; width:220.5pt; v-text-anchor:middle;" arcsize="123%" stroke="false" fillcolor="#007abd"><w:anchorlock/><v:textbox inset="0,0,0,0"><center style="color:#ffffff; font-family:Tahoma, sans-serif; font-size:16px"><![endif]--><a href="' . $url3 . '" style="-webkit-text-size-adjust: none; text-decoration: none; display: inline-block; color: #ffffff; background-color: #007abd; border-radius: 60px; -webkit-border-radius: 60px; -moz-border-radius: 60px; width: auto; width: auto; border-top: 1px solid #007abd; border-right: 1px solid #007abd; border-bottom: 1px solid #007abd; border-left: 1px solid #007abd; padding-top: 15px; padding-bottom: 15px; font-family: Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif; text-align: center; mso-border-alt: none; word-break: keep-all;" target="_blank"><span style="padding-left:15px;padding-right:15px;font-size:16px;display:inline-block;"><span style="font-size: 16px; margin: 0; line-height: 1.2; word-break: break-word; mso-line-height-alt: 19px;">PRE-TRAINING TRADE FORM</span></span></a>';
...
echo json_encode(array('status' => true, 'msg' => "Mail Sent.!"));
